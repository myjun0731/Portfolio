...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .workspace-area { min-height: 16rem; border: 2px dashed #e5e7eb; border-radius: 0.75rem; background: white; position: relative; overflow: hidden; }
        .workspace-area.workspace-filled { border-style: solid; }
        .workspace-area.drag-hover { border-color: #3b82f6; background: #f8fafc; }
        .workspace-hint { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; text-align: center; color: #cbd5e1; font-size: 0.875rem; line-height: 1.6; pointer-events: none; padding: 1rem; }
        .block-workspace-item { position: absolute; padding: 0.5rem 0.75rem; border-radius: 0.75rem; border: 1px solid #d1d5db; background: #ffffff; color: #1f2937; cursor: grab; box-shadow: 0 6px 14px rgba(15,23,42,0.08); transition: box-shadow 0.2s ease, transform 0.2s ease; }
        .block-workspace-item.dragging { opacity: 0.85; cursor: grabbing; transform: scale(1.02); box-shadow: 0 12px 24px rgba(15,23,42,0.14); }
        .block-workspace-item.snapped { transition: left 0.18s ease, top 0.18s ease; }
        .block-workspace-item .workspace-remove { background: none; border: none; color: #6b7280; cursor: pointer; padding: 0; }
        .block-workspace-item .workspace-remove:hover { color: #111827; }
        .block-input { display: inline-flex; align-items: center; justify-content: center; min-width: 2.25rem; padding: 0.15rem 0.45rem; border-radius: 9999px; background: #e5e7eb; color: #1f2937; font-size: 0.75rem; font-weight: 600; }
